...
#################################################################################################
DEVICE='cuda' # xpu if Aurora. Aurora specific - 250306 jubin added
...
def global_local_temporal_contrastive(lsr,gdr, temperature):
    ...
    label = torch.from_numpy(np.asarray(label)).long().to(device=DEVICE) # Aurora specific - 250306 jubin added
...
def rotation_loss(output_rot, target_rot):
    ...
    loss = criterion(output_rot, target_rot.type(torch.LongTensor).to(device=DEVICE)).to(device=DEVICE) # Aurora specific - 250306 jubin added
    return loss
    

def charbonnier_loss(target, pred,epsilon=1e-6, reduction='none'):
    diff = pred - target 
    loss = torch.sqrt(diff ** 2 + epsilon ** 2)
    if reduction == 'none':
        return loss
    else: 
        raise ValueError("charbonnier_loss() only support none reduction mean calculation")


def cauchy_loss(target, pred, scale=1.0, reduction='none'):
    diff = pred - target 
    loss = torch.log(1 + (diff / scale)**2)
    if reduction == 'none':
        return loss
    else: 
        raise ValueError("charbonnier_loss() only support none reduction mean calculation")