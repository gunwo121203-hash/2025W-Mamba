...
class CosineAnnealingWarmUpRestartsMup(_LRScheduler): # 250204 jubin added
    """
        optimizer (Optimizer): Wrapped optimizer.
        first_cycle_steps (int): First cycle step size.
        cycle_mult(float): Cycle steps magnification. Default: -1.
        min_lr_scale(float): Multiplier for settining minimum learning rate. Default: 0.01.
        warmup_steps(int): Linear warmup step size. Default: 0.
        gamma(float): Decrease rate of max learning rate by cycle. Default: 1.
        last_epoch (int): The index of last epoch. Default: -1.
    """
    ...
    def __init__(self,
        ...
        self.first_cycle_steps = first_cycle_steps # first cycle step size
        self.cycle_mult = cycle_mult # cycle steps magnification
        self.min_lr_scale = min_lr_scale # jub added
        self.warmup_steps = warmup_steps # warmup step size
        self.gamma = gamma # decrease rate of max learning rate by cycle
        ...
        super(CosineAnnealingWarmUpRestartsMup, self).__init__(optimizer, last_epoch)
    ...
    def init_lr(self): 
        for i, param_group in enumerate(self.optimizer.param_groups):
            ...
            param_group['lr'] *= self.min_lr_scale # set base_max_lrs, max_lrs, base_lrs using param_group['lr'] instead of assigning absolute values. 250204 jubin added
            self.base_lrs[i] = param_group['lr'] # base_lrs already setup in super().__init__ - 250308 jubin added
        self.base_max_lrs = self.max_lrs.copy()
